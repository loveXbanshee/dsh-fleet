/**
 * dsh-orchard — host entry (formerly dsh-harness-workbench). Punica Studio.
 *
 * JSON API on the DSH web server:
 *
 *   [instance management — v1]
 *   GET  /dsh-orchard/api/state
 *   POST /dsh-orchard/api/scan | add-remote | remove-remote
 *        | stop-local | start-local | set-start-command
 *
 *   [cross-device conversation reading — v0.2]
 *   GET  /dsh-orchard/api/local-sessions | local-session-content
 *   POST /dsh-orchard/api/set-serve-token
 *   GET  /dsh-orchard/api/remote-sessions | remote-session-content   (proxy)
 *   GET  /dsh-orchard/api/sessions | session-content                (token-gated serve)
 *
...
function configFile() {
    return join(homeDir(), 'dsh-orchard.json');
}
...
/** One-time migration: v0.2 kept settings in `dsh-harness-workbench.json`. */
function migrateLegacyConfig() {
    const legacy = join(homeDir(), 'dsh-harness-workbench.json');
    ...
    try {
        ...
        console.error(`[dsh-orchard] config migration from legacy file failed: ${String(error)}`);
    }
}
...
function saveConfig() {
    ...
    try {
        ...
        console.error(`[dsh-orchard] failed to write ${file}: ${String(error)}`);
    }
}
...
async function probe(url, timeoutMs) {
    ...
    try {
        const response = await fetch(url, {
            ...
            redirect: 'manual',
            headers: { 'user-agent': `dsh-orchard/${VERSION}` },
        });
...
async function remoteFetch(remote, path, params) {
    const url = new URL(`${remote.origin}/dsh-orchard/api/${path}`);
    ...
    try {
        const response = await fetch(url.href, { signal: controller.signal, headers: { 'user-agent': `dsh-orchard/${VERSION}` } });
...
function registerAll(webServer) {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/state',
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/scan',
        handler: async (request, response) => {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/add-remote',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/remove-remote',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/stop-local',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/start-local',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/set-start-command',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/local-sessions',
        handler: async (request, response) => {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/local-session-content',
        handler: async (request, response) => {
            if (!guardGetLocal(request, response)) return;
            try {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/set-serve-token',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/sessions',
        handler: async (request, response) => {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/session-content',
        handler: async (request, response) => {
            const auth = tokenAuthorized(request);
            if (!auth.ok) { sendJson(response, auth.code, { error: auth.error }); return; }
            try {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/remote-sessions',
        handler: async (request, response) => {
            if (!guardGetLocal(request, response)) return;
            try {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-orchard/api/remote-session-content',
        handler: async (request, response) => {
            if (!guardGetLocal(request, response)) return;
            try {
                const remote = findRemote(queryParam(request, 'remote'));
