...
// Test-stage self-update sources. First reachable wins; override via DSL_FLEET_UPDATE_URL (semicolon list).
const UPDATE_MANIFEST_URLS = (process.env.DSL_FLEET_UPDATE_URL || '').split(';').filter(Boolean).length > 0
    ? (process.env.DSL_FLEET_UPDATE_URL || '').split(';').filter(Boolean)
    : [
        `https://raw.githubusercontent.com/${UPDATE_REPO}/${UPDATE_BRANCH}/package.json`,
        `https://cdn.jsdelivr.net/gh/${UPDATE_REPO}@${UPDATE_BRANCH}/package.json`,
    ];
...
async function remoteInstanceRows() {
    ...
    return pooled(cfg.remotes, 4, async (remote) => {
        const result = await probe(`${remote.origin}/`, 4000);
        return { id: remote.id, name: remote.name, origin: remote.origin, hasToken: !!(remote.token && remote.token !== ''), online: result.online, dsh: result.dsh, gateway: result.gateway, ms: result.ms, rev: result.rev, error: result.error };
    });
}
...
async function fetchRemoteVersion() {
    ...
    for (const url of UPDATE_MANIFEST_URLS) {
        ...
        try {
            ...
            return { version: remoteVersion, source: url };
        }
        catch (error) {
            lastError = error;
        }
    ...
    const err = new Error(`cannot reach update manifest: ${String(lastError?.message ?? lastError ?? 'network')}`);
...
async function checkForUpdates(force) {
    ...
    try {
        ...
        result = {
            ...
            updateAvailable: !versionAtLeast(VERSION, remote.version),
...
function relaunchScriptContent() {
    ...
    const log = join(homeDir(), 'dsh-fleet-restart.log');
    return [
        ...
        "async function down() { for (let i = 0; i < 30; i++) { try { const r = await fetch('http://127.0.0.1:3080/'); if (r.ok) { await sleep(500); continue; } return; } catch { return; } } }",
        "async function up() { for (let i = 0; i < 90; i++) { await sleep(1000); try { const r = await fetch('http://127.0.0.1:3080/'); if (r.ok) return true; } catch {} } return false; }",
...
async function restartProcess() {
    if (!isWin) throw new Error('one-click restart is Windows-only for now — restart dsh web manually after updating');
    const scriptPath = join(homeDir(), 'dsh-fleet-restart.mjs');
    writeFileSync(scriptPath, relaunchScriptContent(), 'utf8');
    ...
    disposers.push(webServer.register({
        ...
        handler: async (request, response) => {
            ...
            try {
                ...
                const pulled = await pullLatestFromGitHub();
                sendJson(response, 200, { ok: true, updated: true, current: VERSION, latest: check.latest, profileDir: pulled.profileDir });
            }
            catch (error) { sendJson(response, 500, { error: String(error?.message ?? error), hint: 'ensure this device can reach github.com (pnpm/git) and pnpm is installed' }); }
        },
    }));
