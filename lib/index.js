/**
 * dsh-harness-workbench — host entry (v0.2).
 *
 * JSON API on the DSH web server:
 *
 *   [instance management — v1, unchanged]
 *   GET  /dsh-harness-workbench/api/state
 *   POST /dsh-harness-workbench/api/scan | add-remote | remove-remote
 *        | stop-local | start-local | set-start-command
 *
 *   [cross-device conversation reading — v0.2]
 *   GET  /dsh-harness-workbench/api/local-sessions            this machine's index
 *   GET  /dsh-harness-workbench/api/local-session-content     this machine's transcript
 *   POST /dsh-harness-workbench/api/set-serve-token           enable remote reads of this machine
 *   GET  /dsh-harness-workbench/api/remote-sessions           proxy to a registered remote
 *   GET  /dsh-harness-workbench/api/remote-session-content    proxy transcript from a remote
 *
...
export const name = 'dsh-harness-workbench';
...
function configFile() {
    return join(homeDir(), 'dsh-harness-workbench.json');
}
...
function saveConfig() {
    ...
    try {
        ...
        console.error(`[dsh-harness-workbench] failed to write ${file}: ${String(error)}`);
    }
}
...
async function probe(url, timeoutMs) {
    ...
    try {
        const response = await fetch(url, {
            ...
            redirect: 'manual',
            headers: { 'user-agent': `dsh-harness-workbench/${VERSION}` },
        });
...
async function remoteFetch(remote, path, params) {
    const url = new URL(`${remote.origin}/dsh-harness-workbench/api/${path}`);
    ...
    try {
        const response = await fetch(url.href, { signal: controller.signal, headers: { 'user-agent': `dsh-harness-workbench/${VERSION}` } });
...
function registerAll(webServer) {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/state',
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/scan',
        handler: async (request, response) => {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/add-remote',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/remove-remote',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/stop-local',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/start-local',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/set-start-command',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/local-sessions',
        handler: async (request, response) => {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/local-session-content',
        handler: async (request, response) => {
            if (!guardGetLocal(request, response)) return;
            try {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/set-serve-token',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/sessions',
        handler: async (request, response) => {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/session-content',
        handler: async (request, response) => {
            const auth = tokenAuthorized(request);
            if (!auth.ok) { sendJson(response, auth.code, { error: auth.error }); return; }
            try {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/remote-sessions',
        handler: async (request, response) => {
            if (!guardGetLocal(request, response)) return;
            try {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-harness-workbench/api/remote-session-content',
        handler: async (request, response) => {
            if (!guardGetLocal(request, response)) return;
            try {
                const remote = findRemote(queryParam(request, 'remote'));
