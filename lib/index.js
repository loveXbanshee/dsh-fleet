...
function defaultConfig() {
    return {
        ...
        cwd: undefined,
        role: 'controller',     // 'controller' (指挥台, 显示右侧设备栏) | 'managed' (被控端, 仅对外提供会话/重启接口)
...
function loadConfig() {
    ...
    try {
        ...
        config = {
            ...
            role: parsed?.role === 'managed' ? 'managed' : 'controller',
        };
    }
    catch {
...
/** Connect (verification disabled) and capture the server's presented cert chain. */
function grabTlsChain(origin, { timeoutMs = 8000, insecure = false } = {}) {
    return new Promise((resolve, reject) => {
        let url;
        ...
        const socket = tlsConnect({
            ...
            while (current && current.raw && depth < 10) {
                const key = Buffer.from(current.raw).toString('base64');
                ...
                chain.push({
                    ...
                    fingerprint256: fingerprintPem(Buffer.from(current.raw).toString('base64')),
                    pem: current.raw ? `-----BEGIN CERTIFICATE-----\n${Buffer.from(current.raw).toString('base64').match(/.{1,64}/g).join('\n')}\n-----END CERTIFICATE-----\n` : null,
                });
        ...
        socket.on('timeout', () => { socket.destroy(new Error(`timed out after ${timeoutMs}ms`)); });
        socket.on('error', reject);
    });
}

function fingerprintPem(base64Body) {
    try {
        return createHash('sha256').update(Buffer.from(base64Body, 'base64')).digest('hex').toUpperCase().match(/.{2}/g).join(':');
    }
    catch {
        return '';
    }
...
function setRole(role) {
    const cfg = loadConfig();
    cfg.role = role === 'managed' ? 'managed' : 'controller';
    saveConfig();
}
...
async function statePayload() {
    ...
    return {
        ...
        range: { ...cfg.range },
        local: rows, remote,
        role: cfg.role === 'managed' ? 'managed' : 'controller',
...
function registerAll(webServer) {
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-fleet/api/set-role',
        handler: async (request, response) => {
            if (!guardPost(request, response)) return;
            try {
                const body = await readJsonBody(request);
    ...
    disposers.push(webServer.register({
        kind: 'exact', path: '/dsh-fleet/api/remote-cert',
        handler: async (request, response) => {
            if (!guardGetLocal(request, response)) return;
            try {
                const remote = findRemote(queryParam(request, 'remote'));
                if (!remote) { sendJson(response, 404, { error: 'remote not found' }); return; }
                const info = await grabTlsChain(remote.origin, { timeoutMs: 12000, insecure: remote.insecure === true });
                sendJson(response, 200, { ok: true, origin: remote.origin, chain: info.chain });
            }
            catch (error) { sendJson(response, 502, { error: String(error?.message ?? error) }); }
        },
    }));
